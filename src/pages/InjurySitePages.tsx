...
const ImageSlot = ({ label, portrait = false }: { label: string; portrait?: boolean }) => (
  <div className={`${portrait ? "h-[34svh] min-h-[240px] max-h-[360px]" : "h-[42svh] min-h-[280px] max-h-[440px]"} relative w-full overflow-hidden border border-foreground/10 bg-[#e8e3da] text-foreground`}>
    <div className="absolute inset-0 opacity-50" style={{ backgroundImage: "linear-gradient(135deg, rgba(31,27,23,.025) 25%, transparent 25%, transparent 50%, rgba(31,27,23,.025) 50%, rgba(31,27,23,.025) 75%, transparent 75%)", backgroundSize: "28px 28px" }} />
    <div className="absolute inset-x-4 top-4 flex items-center justify-between text-[8px] font-medium uppercase tracking-[0.16em] text-foreground/32"><span>Image space</span><span>↗</span></div>
    <div className="absolute inset-x-4 bottom-4 max-w-[78%] text-[10px] leading-4 text-foreground/44">{label}</div>
  </div>
);
...
const TopicSection = ({ locale, number, eyebrow, title, body, children, warm = false }: { locale: SiteLocale; number: string; eyebrow: string; title: string; body?: string; children?: ReactNode; warm?: boolean }) => (
  <section className={`flex min-h-[100svh] items-center py-14 md:py-16 lg:py-12 ${warm ? "bg-[#f3efe8]" : "bg-background"}`}>
...
const PhoneCta = ({ locale }: { locale: SiteLocale }) => (
  <section className="flex min-h-[100svh] items-center bg-[#171717] py-14 text-[#f3eee5] md:py-16">
    <div className="site-shell w-full">
      <div className="grid gap-8 border-t border-white/12 pt-6 lg:grid-cols-[0.3fr_1.25fr_0.45fr] lg:items-end lg:gap-12 xl:gap-16">
        <div className="text-[9px] font-medium uppercase tracking-[0.16em] text-white/36">{isKo(locale) ? "상담" : "Consultation"}</div>
        <div className="max-w-[760px]"><h2 style={serif(locale)} className={`${isKo(locale) ? "text-[clamp(1.8rem,2.45vw,2.65rem)] font-medium leading-[1.36] tracking-[-0.04em]" : "editorial-serif text-[clamp(1.95rem,2.5vw,2.8rem)] leading-[1.06] tracking-[-0.024em]"}`}>{isKo(locale) ? "사고 이후 무엇을 해야 하는지 명확하게 정리하세요." : "Get a clear answer on what should happen next."}</h2><p className="mt-4 max-w-[560px] text-[12px] leading-6 text-white/48">{isKo(locale) ? "현재 상황을 간단히 알려주시면 상담 가능 여부와 다음 단계를 확인할 수 있습니다." : "Share the outline of what happened and the firm can determine whether the matter is a fit and what the next step should be."}</p></div>
...
const PracticeGrid = ({ locale, limit = 8 }: { locale: SiteLocale; limit?: number }) => (
  <div className="grid border-t border-foreground/12 sm:grid-cols-2 xl:grid-cols-4">{practiceAreas.slice(0, limit).map((practice, index) => <Link key={practice.slug} to={`${prefixFor(locale)}/practice-areas/${practice.slug}`} className="group min-h-[132px] border-b border-foreground/12 py-5 sm:px-6 sm:first:pl-0 xl:border-l xl:first:border-l-0"><div className="flex items-center justify-between text-[9px] text-foreground/26"><span>{String(index + 1).padStart(2, "0")}</span><ArrowRight className="h-3 w-3 transition-transform group-hover:translate-x-1" /></div><div style={serif(locale)} className={`${isKo(locale) ? "mt-7 text-[1.22rem] font-medium" : "editorial-serif mt-7 text-[1.42rem]"}`}>{isKo(locale) ? practice.koTitle : practice.title}</div><p className="mt-2 max-w-[300px] text-[10px] leading-4 text-foreground/44">{isKo(locale) ? practice.koDescription : practice.description}</p></Link>)}</div>
);
...
const NextSteps = ({ locale }: { locale: SiteLocale }) => (
  <div className="grid border-t border-foreground/12 md:grid-cols-3">
    ...
    ].map(([label, body, path], index) => <Link key={path} to={`${prefixFor(locale)}${path}`} className="group min-h-[170px] border-b border-foreground/12 py-5 md:border-l md:px-7 md:first:border-l-0 md:first:pl-0"><div className="text-[9px] text-foreground/25">0{index + 1}</div><div className="mt-9 flex items-center justify-between text-[13px] font-medium"><span>{label}</span><ArrowRight className="h-3 w-3 transition-transform group-hover:translate-x-1" /></div><p className="mt-3 max-w-[300px] text-[11px] leading-5 text-foreground/46">{body}</p></Link>)}
  </div>
);
