...
const ThankYou = ({ locale = "en" }: ThankYouProps) => {
  ...
  const NavigationComponent = isKorean ? KoreanNavigation : isSpanish ? SpanishNavigation : Navigation;
  const FooterComponent = isKorean ? KoreanFooter : isSpanish ? SpanishFooter : Footer;
  const homeHref = isKorean ? "/ko" : isSpanish ? "/es" : "/";
          ...
          <div className="grid gap-12 border-t border-[#f3eee5]/14 pt-7 lg:grid-cols-[0.72fr_1.28fr] lg:gap-20">
            <div>
              <span className="text-[10px] font-medium uppercase tracking-[0.18em] text-[#f3eee5]/42">
                {isKorean ? "문의 접수 완료" : isSpanish ? "Mensaje recibido" : "Message received"}
              </span>
            </div>
            ...
            <div className="max-w-[760px]">
              ...
              >
                {isKorean ? "문의 내용을 잘 받았습니다." : isSpanish ? "Hemos recibido su mensaje." : "We’ve received your message."}
              </h1>

              <p className="mt-6 max-w-[600px] text-[15px] leading-7 text-[#f3eee5]/62 md:text-[16px]">
                {isKorean
                  ? "보내주신 내용을 확인한 뒤 담당자가 곧 연락드리겠습니다."
                  : isSpanish ? "Gracias por comunicarse. Howard revisará lo que envió y la firma se pondrá en contacto con usted pronto." : "Thanks for reaching out. Howard will review what you sent and get back to you soon."}
              </p>
              ...
              <div className="mt-10 border-t border-[#f3eee5]/12 pt-6">
                <p className="max-w-[560px] text-[12px] leading-6 text-[#f3eee5]/42">
                  {isKorean
                    ...
                    : isSpanish ? "Si su asunto es urgente o sensible al tiempo, llame directamente a la oficina en lugar de esperar una respuesta en línea." : "If your matter is urgent or time-sensitive, please call the office rather than waiting for an online response."}
                </p>
                ...
                <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:items-center">
                  ...
                  >
                    ...
                    <span className="relative z-10">{isKorean ? "홈으로 돌아가기" : isSpanish ? "Volver al inicio" : "Back to home"}</span>
                  </a>
                  <a
