...
const es = (locale: SiteLocale) => locale === "es";
const prefix = (locale: SiteLocale) => ko(locale) ? "/ko" : es(locale) ? "/es" : "";
...
const Frame = ({ locale, children }: { locale: SiteLocale; children: React.ReactNode }) => (
  <div className="min-h-screen overflow-x-clip bg-[#F9F8F6] text-[#1E1C1A]" style={ko(locale) ? { fontFamily: '"Noto Sans KR", sans-serif' } : undefined}>
    {ko(locale) ? <KoreanNavigation /> : es(locale) ? <SpanishNavigation /> : <Navigation />}
    {children}
    {ko(locale) ? <KoreanFooter /> : es(locale) ? <SpanishFooter /> : <Footer />}
  </div>
);
...
const ProfileLinks = ({ locale }: { locale: SiteLocale }) => (
  <div className="mt-8 border-t border-white/14 pt-5">
    <div className="mb-3 text-[8px] font-semibold uppercase tracking-[0.16em] text-white/35">{ko(locale) ? "업무 초점" : es(locale) ? "Enfoque de práctica" : "Practice focus"}</div>
    <div className="flex flex-wrap gap-x-5 gap-y-2 text-[11px] text-white/62">
      <span>{ko(locale) ? "개인상해" : es(locale) ? "Lesiones personales" : "Personal injury"}</span>
      <span>{ko(locale) ? "교통사고" : es(locale) ? "Reclamos por accidentes" : "Accident claims"}</span>
      <span>Buena Park</span>
      <span>{ko(locale) ? "Orange County 인근" : es(locale) ? "Comunidades cercanas" : "Nearby communities"}</span>
    </div>
  </div>
);
      ...
      <section className="attorney-hero bg-[#211A16] pt-[60px] text-[#F3EEE5]">
        <div className="grid min-h-[calc(100svh-60px)] lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="flex min-h-[540px] flex-col justify-between gap-8 px-6 py-10 sm:px-10 lg:px-12 lg:py-12 xl:px-16">
            <div className="flex items-center justify-between border-t border-white/16 pt-4"><Eyebrow light>{ko(locale) ? "변호사" : es(locale) ? "Abogado" : "Attorney"}</Eyebrow><span className="text-[9px] uppercase tracking-[0.14em] text-white/34">Buena Park · California</span></div>
            <motion.div initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
              <PageBreadcrumb locale={locale} title={ko(locale) ? "변호사" : es(locale) ? "Abogado" : "Attorney"} />
              ...
              <p className="mt-7 max-w-[500px] text-base leading-7 text-white/80">{ko(locale) ? "부에나파크의 Howard Choi 변호사를 만나보세요. 사고와 관련된 질문을 상담할 수 있습니다." : es(locale) ? "Conozca a Howard Choi, abogado de lesiones personales en Buena Park. Obtenga respuestas sobre su reclamo por accidente." : "Meet Howard Choi, a personal injury attorney in Buena Park. Get answers to your questions about an accident claim."}</p>
              <div className="mt-8 flex flex-wrap gap-3">
                <a href={`${prefix(locale)}/contact`} className="rounded-full bg-[#F3EEE5] px-6 py-3 text-[14px] font-semibold text-[#1E1C1A]">{ko(locale) ? "상담 예약" : es(locale) ? "Programar una consulta" : "Schedule a consultation"}</a>
                <a href="#profile" className="inline-flex min-h-11 items-center gap-2 rounded-full border border-white/30 px-6 py-3 text-sm text-white">{ko(locale) ? "프로필 보기" : es(locale) ? "Ver perfil" : "View profile"}<ArrowRight className="h-4 w-4" /></a>
            ...
            <div className="grid gap-2 border-t border-white/14 pt-4 text-[9px] uppercase tracking-[0.13em] text-white/34 sm:grid-cols-2"><span>{brand.address}</span><span className="sm:text-right">{ko(locale) ? "개인상해 · 사고" : es(locale) ? "Lesiones personales · Accidentes" : "Personal injury · Accidents"}</span></div>
          </div>
        </div>
      </section>
