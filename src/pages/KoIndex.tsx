...
const processSteps = [
  {
    title: "상담",
    body: "사고 경위, 부상 상태, 현재 가지고 있는 정보와 지금 가장 먼저 확인해야 할 문제를 정리하는 직접적인 첫 상담입니다.",
    points: ["사고 경위와 현재 상황 파악", "당장 필요한 우선순위 확인"],
    image: "https://images.unsplash.com/photo-1450101499163-c8848c66ca85?auto=format&fit=crop&w=1400&q=88",
  },
  {
    title: "조사",
    body: "사고와 관련된 증거, 당사자, 기록, 보험 정보를 확인하고 나중에 필요한 자료가 사라지지 않도록 정리합니다.",
    points: ["기록과 가능한 증거 확보", "책임과 보험 범위 확인"],
    image: "https://images.unsplash.com/photo-1551836022-d5d88e9218df?auto=format&fit=crop&w=1400&q=88",
  },
  {
    title: "청구 준비",
    body: "치료비만이 아니라 회복 과정, 업무 손실, 일상생활 변화 등 부상이 실제 삶에 미친 영향을 함께 정리합니다.",
    points: ["치료와 회복 과정 기록", "업무와 일상생활 영향 정리"],
    image: "https://images.unsplash.com/photo-1521791055366-0d553872125f?auto=format&fit=crop&w=1400&q=88",
  },
  {
    title: "해결",
    body: "자료와 선택지를 정리한 상태에서 가능한 해결 경로를 검토하고 다음 결정을 명확하게 이어갑니다.",
    points: ["가능한 해결 경로 검토", "다음 단계 명확화"],
...
const HeroKo = () => {
  ...
  return (
    <section className="relative h-[100svh] min-h-[700px] w-full overflow-hidden bg-[#17130f] md:min-h-[680px]">
      ...
      <motion.div className="absolute inset-0 z-10 flex items-end" style={shouldReduceMotion ? undefined : { opacity: contentOpacity, filter: contentFilter, y: contentY }}>
        <div className="site-shell pb-16 sm:pb-20 md:pb-28">
          <div className="max-w-[760px] text-[#f3eee5]">
            <motion.p initial={shouldReduceMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18, duration: 0.7 }} className="mb-4 text-[12px] font-medium text-[#f3eee5]/88 md:text-[13px]">
              사고 · 상해 법률상담
            </motion.p>

            <motion.h1 initial={shouldReduceMotion ? false : { opacity: 0, y: 16, filter: "blur(9px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} transition={{ delay: 0.27, duration: 0.9, ease: [0.22, 1, 0.36, 1] }} style={koSerif} className="text-[clamp(2.5rem,10.5vw,4.35rem)] font-medium leading-[1.14] tracking-[-0.05em] text-[#f3eee5]">
              부에나파크 사고 변호사.
            </motion.h1>

            <motion.p initial={shouldReduceMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.7 }} className="mt-5 max-w-[590px] text-[14px] leading-7 text-[#f3eee5]/74 md:text-[15px]">
              부에나파크와 인근 지역에서 사고로 부상을 입은 분들을 위해 명확하고 신속한 법률 대응을 제공합니다.
            </motion.p>

            <motion.div initial={shouldReduceMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.7 }} className="mt-7 flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
              <button onClick={() => document.getElementById("booking")?.scrollIntoView({ behavior: "smooth" })} className="liquid-cta inline-flex w-full items-center justify-center gap-3 rounded-full px-6 py-3.5 text-[12px] font-medium sm:w-fit sm:py-3 sm:text-[13px]">
                <span className="relative z-10">상담 요청</span><ArrowRight className="relative z-10 h-4 w-4" />
              </button>
              <a href="tel:+17146900007" className="text-[12px] text-[#f3eee5]/82 transition-opacity hover:opacity-70 md:text-[13px]">전화 +1 714-690-0007</a>
            </motion.div>

            <motion.div initial={shouldReduceMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.62, duration: 0.7 }} className="mt-6 flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-[#f3eee5]/16 pt-4 text-[10px] tracking-[0.04em] text-[#f3eee5]/56 sm:text-[11px]">
              <span>California Bar #284364</span><span className="hidden h-3 w-px bg-[#f3eee5]/16 sm:block" /><span>2012년 등록</span>
            </motion.div>
          </div>
        </div>
      </motion.div>

      <motion.div className="absolute inset-x-0 bottom-5 z-20 md:bottom-6" style={shouldReduceMotion ? undefined : { opacity: contentOpacity }}>
    ...
    <section id="approach" ref={ref} className="bg-background py-16 md:py-20 lg:flex lg:min-h-[100svh] lg:items-center lg:py-[clamp(3rem,6vh,5rem)]">
      <div className="site-shell w-full">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.72 }} className="max-w-[670px]">
          <span className="mb-4 block text-[13px] font-semibold leading-none text-foreground md:mb-5 md:text-[14px]">진행 방식</span>
          <h2 style={koSerif} className="text-[clamp(2.35rem,10vw,3.75rem)] font-medium leading-[1.2] tracking-[-0.045em]">명확한 다음 단계.</h2>
          <p className="mt-4 max-w-[540px] text-[14px] leading-7 text-foreground/58 md:mt-5 md:text-[clamp(1rem,1.35vw,1.2rem)]">첫 상담부터 조사, 자료 정리, 해결까지 각 단계가 왜 필요한지 분명하게 설명합니다.</p>
        </motion.div>
        ...
        <div className="mt-10 grid gap-4 md:mt-12 lg:mt-[clamp(2.8rem,6vh,4.8rem)] lg:h-[min(59vh,600px)] lg:min-h-[500px] lg:grid-cols-2 lg:gap-5">
          <motion.div initial={{ opacity: 0, x: -16 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8, delay: 0.05 }} className="relative min-h-[280px] overflow-hidden rounded-[4px] bg-neutral-200 sm:min-h-[340px] lg:h-full lg:min-h-0">
            <AnimatePresence mode="wait" initial={false}><motion.img key={processSteps[active].image} src={processSteps[active].image} alt={`${processSteps[active].title} 과정`} initial={{ opacity: 0, scale: 1.018, filter: "blur(3px)" }} animate={{ opacity: 1, scale: 1, filter: "blur(0px)" }} exit={{ opacity: 0 }} transition={{ duration: 0.58, ease: [0.22, 1, 0.36, 1] }} className="absolute inset-0 h-full w-full object-cover" /></AnimatePresence><div className="absolute inset-0 bg-black/[0.06]" />
          </motion.div>
            ...
            {processSteps.map((step, index) => {
              ...
              return (
                <motion.article key={step.title} layout transition={{ duration: 0.55, ease: [0.22, 1, 0.36, 1] }} className="relative min-h-0 overflow-hidden rounded-[4px] bg-[#e6e4e1]">
                  <button type="button" onClick={() => setActive(index)} className="flex h-full w-full min-h-0 flex-col px-5 py-5 text-left sm:px-7 sm:py-6 md:px-8" aria-expanded={expanded}>
                    <div className="flex w-full items-start justify-between gap-6"><span style={koSerif} className="text-[clamp(1.55rem,7vw,2.35rem)] font-medium leading-[1.2] tracking-[-0.04em]">{step.title}</span>{expanded ? <X className="mt-1 h-5 w-5 shrink-0 stroke-[1.65]" /> : <Plus className="mt-1 h-5 w-5 shrink-0 stroke-[1.65]" />}</div>
                    <AnimatePresence initial={false}>{expanded && <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.3, delay: 0.05 }} className="mt-auto max-w-[480px] pt-4 sm:pt-5"><p className="text-[13px] leading-6 text-foreground/88 sm:text-[14px] sm:leading-7">{step.body}</p><div className="mt-3 space-y-2 sm:mt-4 sm:space-y-2.5">{step.points.map((point) => <div key={point} className="flex items-center gap-3 text-[12px] text-foreground/68 sm:text-[13px]"><Check className="h-4 w-4 shrink-0 stroke-[1.65]" /><span>{point}</span></div>)}</div></motion.div>}</AnimatePresence>
                  </button>
                  {expanded && isInView && <motion.div key={`ko-progress-${active}`} className="absolute bottom-0 left-0 h-[2px] w-full origin-left bg-foreground" initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: AUTO_ADVANCE_MS / 1000, ease: "linear" }} />}
                </motion.article>
              );
            })}
          </motion.div>
    ...
    <section ref={ref} className="flex min-h-[100svh] items-center bg-[#171717] py-16 text-white md:py-20 lg:py-0">
      <div className="site-shell grid w-full gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:gap-16">
        <motion.div initial={{ opacity: 0, y: 22 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.75 }} className="lg:py-10">
          <span className="mb-5 block text-[10px] tracking-[0.14em] text-white/42">직접적인 소통</span>
          <h2 style={koSerif} className="text-[clamp(2.4rem,10vw,4rem)] font-medium leading-[1.18] tracking-[-0.045em] text-white">직접 소통.<br />중간 단계 없이.</h2>
          <p className="mt-6 max-w-[460px] text-[14px] leading-7 text-white/58 md:text-[15px]">상해 청구는 가능한 한 담당 변호사 가까이에서 진행됩니다. 불필요한 전달 단계를 줄이고 상황과 다음 결정을 더 직접적으로 이해할 수 있도록 합니다.</p>
          <div className="mt-10 grid max-w-[420px] grid-cols-2 gap-6 border-t border-white/12 pt-6 sm:mt-14 sm:gap-10 sm:pt-7">
            <div><div style={koSerif} className="text-[2.25rem] leading-none sm:text-[2.4rem]">1:1</div><div className="mt-2 text-[9px] tracking-[0.08em] text-white/38 sm:text-[10px]">직접 변호사 소통</div></div>
            <div><div style={koSerif} className="text-[2.25rem] leading-none sm:text-[2.4rem]">08</div><div className="mt-2 text-[9px] tracking-[0.08em] text-white/38 sm:text-[10px]">상해 업무 분야</div></div>
          </div>
        </motion.div>
        <motion.div initial={{ opacity: 0, scale: 0.985 }} animate={isInView ? { opacity: 1, scale: 1 } : {}} transition={{ duration: 0.9, delay: 0.08 }} className="ml-auto w-full overflow-hidden rounded-[2px] lg:h-[72svh] lg:max-h-[760px] lg:min-h-[560px]">
          <img src={leadCounselImage} alt="사무실의 변호사" loading="lazy" decoding="async" className="aspect-[4/3] h-full w-full object-cover object-center grayscale-[15%] sm:aspect-[16/10] lg:aspect-auto" />
        </motion.div>
      </div>
    </section>
  );
...
const FAQKo = () => {
  ...
  return (
    <section id="faq" ref={ref} className="bg-background pb-28 pt-24 md:pb-36 md:pt-28 lg:pb-44">
      <div className="site-shell">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7 }} className="mx-auto mb-12 max-w-2xl text-center">
          ...
          <h2 style={koSerif} className="text-[clamp(2.2rem,9vw,3.4rem)] font-medium leading-[1.25] tracking-[-0.04em]">사고 이후 자주 묻는 질문.</h2>
        </motion.div>
    ...
    <section id="booking" ref={ref} className="relative isolate flex min-h-[100svh] items-center overflow-hidden bg-[#171717] py-20 text-white md:py-24">
      ...
      <div className="site-shell w-full">
        <motion.div initial={{ opacity: 0, y: 22 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.75 }} className="mx-auto max-w-[790px] text-center">
          <span className="mb-5 block text-[12px] text-white/52">사고 이후 다음 단계</span>
          <h2 style={koSerif} className="text-[clamp(2.3rem,9.5vw,3.9rem)] font-medium leading-[1.2] tracking-[-0.045em]">상황을 설명하는 것부터 시작하세요.</h2>
          <p className="mx-auto mt-6 max-w-[600px] text-[14px] leading-7 text-white/60 md:text-[15px]">사고 경위와 현재 상황을 알려주시면 상담 가능 여부와 다음 단계에 필요한 기본 정보를 확인할 수 있습니다.</p>
          <div className="mt-9 flex flex-col items-center justify-center gap-4 sm:flex-row"><a href="/ko/contact" className="liquid-cta inline-flex w-full items-center justify-center gap-3 rounded-full px-6 py-3.5 text-[12px] font-medium sm:w-fit sm:py-3 sm:text-[13px]"><span className="relative z-10">상담 요청</span><ArrowRight className="relative z-10 h-4 w-4" /></a><a href="tel:+17146900007" className="text-[13px] text-white/68 hover:text-white">전화 +1 714-690-0007</a></div>
