...
export function GuideAttorney({ locale }: { locale: SiteLocale }) {
  ...
  return <section className="guide-attorney bg-[#eae4db] px-5 py-14 md:py-20">
    <div className="mx-auto grid max-w-[1120px] items-center gap-8 md:grid-cols-[0.7fr_1.3fr] md:gap-14">
      ...
      <div><p className="mb-4 text-xs font-semibold uppercase tracking-[0.12em] text-[#67584a]">{ko ? "변호사 소개" : "Meet your attorney"}</p><h2 className="text-[clamp(2rem,3.5vw,3.4rem)] font-semibold leading-tight tracking-[-0.04em]">Howard Choi</h2><p className="mt-3 text-sm font-medium text-[#57514b]">{ko ? "개인상해 · 부에나파크" : "Personal injury · Buena Park"}</p><p className="mt-5 max-w-[540px] text-base leading-7 text-[#57514b]">{ko ? "사고 경위와 궁금한 점을 알려주세요. 프로필을 살펴보거나 상담을 요청할 수 있습니다." : "Get to know the attorney behind the firm. Review his profile or request a conversation about your accident."}</p>
        <div className="mt-7 flex flex-wrap gap-4"><a href={`${prefix}/attorney#profile`} className="inline-flex min-h-11 items-center gap-3 rounded-full bg-[#211c17] px-6 py-3 text-sm font-medium text-[#f3eee5]">{ko ? "프로필 보기" : "View attorney profile"}<ArrowRight aria-hidden="true" className="h-4 w-4" /></a><a href={`${prefix}/contact`} className="inline-flex min-h-11 items-center gap-3 rounded-full border border-[#211c17]/30 px-6 py-3 text-sm font-medium">{ko ? "상담 요청" : "Request a consultation"}</a></div>
      </div>
    </div>
  </section>;
}
