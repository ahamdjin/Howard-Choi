  ...
  return (
    <section id="approach" ref={ref} className="flex min-h-[100svh] items-center bg-background py-[clamp(3rem,6vh,5rem)]">
      <div className="site-shell w-full">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.72 }} className="max-w-[620px]"><span className="mb-5 block text-[15px] font-semibold leading-none text-foreground">How we work</span><h2 className="editorial-serif text-[clamp(3.15rem,4.3vw,4.25rem)] leading-[0.96] tracking-[-0.025em]">A clear path forward.</h2><p className="mt-5 max-w-[520px] text-[clamp(1.08rem,1.5vw,1.32rem)] leading-[1.28] text-foreground/58">From the first conversation through investigation, documentation, and resolution.</p></motion.div>
        <div className="mt-[clamp(2.8rem,6vh,4.8rem)] grid gap-5 lg:h-[min(59vh,600px)] lg:min-h-[500px] lg:grid-cols-2">
          ...
          <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: isInView ? 1 : 0, x: isInView ? 0 : 16, gridTemplateRows: rowTemplate }} transition={{ opacity: { duration: 0.8, delay: 0.09 }, x: { duration: 0.8, delay: 0.09 }, gridTemplateRows: { duration: 0.62, ease: [0.22, 1, 0.36, 1] } }} className="grid h-[560px] min-h-0 gap-3 lg:h-full">
            {steps.map((step, index) => { const expanded = active === index; return <motion.article key={step.title} layout transition={{ duration: 0.55, ease: [0.22, 1, 0.36, 1] }} className="relative min-h-0 overflow-hidden rounded-[4px] bg-[#e6e4e1]"><button type="button" onClick={() => setActive(index)} className="flex h-full w-full min-h-0 flex-col px-7 py-6 text-left md:px-8" aria-expanded={expanded}><div className="flex w-full items-start justify-between gap-6"><span className="editorial-serif text-[clamp(2rem,2.75vw,2.75rem)] leading-none tracking-[-0.02em]">{step.title}</span>{expanded ? <X className="mt-1 h-5 w-5 shrink-0 stroke-[1.65]" /> : <Plus className="mt-1 h-5 w-5 shrink-0 stroke-[1.65]" />}</div><AnimatePresence initial={false}>{expanded && <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.3, delay: 0.05 }} className="mt-auto max-w-[470px] pt-5"><p className="text-[15px] leading-6 text-foreground/88">{step.body}</p><div className="mt-4 space-y-2.5">{step.points.map((point) => <div key={point} className="flex items-center gap-3 text-[13px] text-foreground/68"><Check className="h-4 w-4 shrink-0 stroke-[1.65]" /><span>{point}</span></div>)}</div></motion.div>}</AnimatePresence></button>{expanded && isInView && <motion.div key={`progress-${active}`} className="absolute bottom-0 left-0 h-[2px] w-full origin-left bg-foreground" initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: AUTO_ADVANCE_MS / 1000, ease: "linear" }} />}</motion.article>; })}
          </motion.div>
        </div>
      </div>
    </section>
