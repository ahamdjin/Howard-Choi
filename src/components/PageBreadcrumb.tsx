...
export default function PageBreadcrumb({ locale, title }: { locale: SiteLocale; title: string }) {
  ...
  const prefix = ko ? "/ko" : "";
  const parent = [
    { path: "/practice-areas", label: ko ? "업무 분야" : "Practice Areas" },
    { path: "/locations", label: ko ? "지역" : "Locations" },
    { path: "/blogs", label: ko ? "블로그" : "Blogs" },
  ].find((item) => pathname.startsWith(prefix + item.path + "/"));
  return (
    <nav aria-label={ko ? "페이지 경로" : "Breadcrumb"} className="mb-6 text-xs leading-5">
      <ol className="flex flex-wrap items-center gap-x-2 gap-y-1">
        <li><a href={prefix || "/"} className="underline underline-offset-4">{ko ? "홈" : "Home"}</a></li>
