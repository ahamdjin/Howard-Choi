...
const SiteNavigation = ({ locale }: { locale: SiteLocale }) => {
  ...
  const PanelLink = ({ number, title, body, link }: { number: string; title: string; body?: string; link: string }) => (
    <a href={link} onClick={closeAll} className="group flex min-h-[76px] flex-col justify-between border-b border-foreground/12 py-4 sm:px-5 sm:first:pl-0 xl:min-h-[88px] xl:border-l xl:first:border-l-0">
      ...
      <motion.nav initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.55, ease: [0.22, 1, 0.36, 1] }} onKeyDown={(event) => { if (event.key === "Escape") { const trigger = event.currentTarget.querySelector<HTMLButtonElement>('[aria-expanded="true"]'); closeAll(); trigger?.focus(); } }} onPointerLeave={(event) => { if (event.pointerType === "mouse") setPanel(null); }} onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setPanel(null); }} className="fixed left-[5px] right-[5px] top-0 z-[120] overflow-hidden bg-background text-foreground sm:left-[7px] sm:right-[7px]" style={{ fontFamily: ko ? '"Noto Sans KR", sans-serif' : '"Inter", Arial, sans-serif' }}>
        ...
        <AnimatePresence initial={false}>{mobileOpen && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "calc(100svh - 60px)", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.38, ease: [0.22, 1, 0.36, 1] }} className="overflow-hidden border-t border-foreground/[0.08] bg-background text-foreground lg:hidden"><div className="site-shell flex h-full flex-col overflow-y-auto pb-6 pt-3">
          <div>
            ...
            <AnimatePresence initial={false}>{mobileGroup === "practice" && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden border-b border-foreground/10"><div className="grid grid-cols-2 gap-x-5 gap-y-3 py-5 text-[14px] text-foreground/80">{practiceAreas.map((practice) => <a key={practice.slug} href={href(`/practice-areas/${practice.slug}`)} onClick={closeAll} className="leading-5">{ko ? practice.koTitle : practice.title}</a>)}</div></motion.div>}</AnimatePresence>
            ...
            <AnimatePresence initial={false}>{mobileGroup === "locations" && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden border-b border-foreground/10"><div className="grid grid-cols-2 gap-x-5 gap-y-3 py-5 text-[14px] text-foreground/80">{serviceLocations.map((item) => <a key={item.slug} href={href(`/locations/${item.slug}`)} onClick={closeAll}>{ko ? item.koName : item.name}</a>)}</div></motion.div>}</AnimatePresence>
            {[[labels.attorneys, href("/attorney")], [labels.results, href("/results")], [labels.blog, href("/blogs")], [ko ? "예상 배상액 계산기" : "Case Value Calculator", href("/case-value-calculator")], [labels.about, href("/about")], [labels.contact, href("/contact")]].map(([label, itemHref]) => <a key={itemHref} href={itemHref} aria-current={isCurrent(itemHref) ? "page" : undefined} onClick={closeAll} className="block border-b border-foreground/10 py-4 text-[1.05rem] font-medium tracking-[-0.025em]">{label}</a>)}
          </div>
