  ...
  <section id="results" aria-labelledby="home-results-heading" className="bg-[#0f0f0f] py-20 text-[#f4f1ea] md:py-24 lg:py-28">
    <div className="site-shell">
      <div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end lg:gap-16">
        <div>
          <div className="text-[10px] uppercase tracking-[0.16em] text-white/38">Results &amp; experience</div>
      ...
      <div className="mt-12 grid gap-4 md:grid-cols-3 lg:mt-14">
        {results.map((result) => (
          <div key={result.title} className="overflow-hidden rounded-[3px] bg-[#191919]">
            ...
            <div className="p-5">
              <div className="text-[13px] font-medium tracking-[-0.01em] text-white">{result.title}</div>
              <p className="mt-2 text-[12px] leading-5 text-white/52">{result.detail}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <p className="mt-9 max-w-[760px] text-[10px] leading-4 text-white/30">
        Prior results do not guarantee a similar outcome. Every matter depends on its own facts, damages, insurance,
