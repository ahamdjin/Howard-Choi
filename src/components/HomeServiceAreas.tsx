    ...
    <div className="site-shell">
      <div className="grid gap-10 lg:grid-cols-[1fr_auto] lg:items-end lg:gap-16">
        <div className="max-w-[860px]">
          <div className="text-[10px] uppercase tracking-[0.16em] text-foreground/36">{ko ? "서비스 지역" : es ? "Zonas de servicio" : "Areas served"}</div>
          <h2 id="home-service-areas-heading" className="editorial-serif mt-5 text-[clamp(2.6rem,5vw,5.2rem)] leading-[0.92] tracking-[-0.04em]">
            {ko ? <>부에나파크를 기반으로,<br /><span className="text-foreground/38">인근 지역까지 지원합니다.</span></> : es ? <>Con sede en Buena Park.<br /><span className="text-foreground/38">Atendiendo a las comunidades cercanas.</span></> : <>Based in Buena Park.<br /><span className="text-foreground/38">Serving the communities around it.</span></>}
          </h2>
          <p className="mt-7 max-w-[560px] text-[14px] leading-7 text-foreground/52">
            {ko ? "사무실은 부에나파크 Beach Blvd에 있으며, 북부 오렌지카운티와 인근 로스앤젤레스카운티 지역의 사고·개인상해 사건을 지원합니다." : es ? "La oficina está en Beach Blvd en Buena Park, con representación en accidentes y lesiones personales disponible en el norte del Condado de Orange y comunidades cercanas del Condado de Los Ángeles." : "The office is on Beach Blvd in Buena Park, with accident and personal injury representation available across North Orange County and nearby Los Angeles County communities."}
          </p>
        </div>
        <a
        ...
        >
          {ko ? "전체 지역 보기" : es ? "Ver todas las zonas de servicio" : "View all service areas"} <ArrowUpRight className="h-3.5 w-3.5" />
        </a>
      </div>
          ...
          >
            ...
            <div className="relative rounded-[2px] bg-[#15110d]/55 px-4 py-3.5 backdrop-blur-[6px]">
              ...
              <div className="mt-2.5 border-t border-[#f3eee5]/20 pt-2.5 text-[11px] leading-4 text-[#f3eee5]/78">
                {ko ? `${location.ots.year}년 교통사고 ${location.ots.total.toLocaleString()}건` : es ? `${location.ots.total.toLocaleString()} colisiones reportadas · ${location.ots.year}` : `${location.ots.total.toLocaleString()} reported collisions · ${location.ots.year}`}
                <span className="mt-0.5 block text-[10px] text-[#f3eee5]/52">{ko ? "캘리포니아 교통안전국(OTS)" : es ? "California OTS" : "California OTS"}</span>
              </div>
            </div>
          </a>
        ))}
