...
const MorphCard = ({ practice, index, columns, detailOpacity, overlayOpacity }: MorphCardProps) => (
  <motion.div className="relative grid h-full overflow-hidden rounded-[4px] bg-[#e8e4de] shadow-[0_24px_70px_rgba(29,24,19,0.14)]" style={columns ? { gridTemplateColumns: columns } : undefined}>
    <div className="relative min-w-0 overflow-hidden"><img src={practice.image} alt={practice.alt} className="absolute inset-0 h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-[#15110d]/74 via-[#17130f]/7 to-transparent" /><motion.div className="absolute inset-x-0 bottom-0 z-10 p-6 text-[#f3eee5] md:p-7" style={overlayOpacity ? { opacity: overlayOpacity } : { opacity: 1 }}><div className="mb-2 text-[10px] tracking-[0.12em] text-[#f3eee5]/55">0{index + 1}</div><h3 style={koSerif} className="text-[clamp(1.65rem,2.1vw,2.15rem)] font-medium leading-[1.18] tracking-[-0.04em]">{practice.title}</h3><p className="mt-2 max-w-[330px] text-[13px] leading-6 text-[#f3eee5]/72">{practice.description}</p></motion.div></div>
    <motion.div className="min-w-0 overflow-hidden bg-[#e8e4de]" style={detailOpacity ? { opacity: detailOpacity } : undefined}><div className="flex h-full min-w-[300px] flex-col justify-between px-8 py-8 text-[#211c17] xl:px-10 xl:py-10"><div><div className="mb-8 text-[12px] tracking-[0.1em] text-[#211c17]/45">0{index + 1}</div><h3 style={koSerif} className="max-w-[330px] text-[clamp(2.3rem,3.4vw,3.8rem)] font-medium leading-[1.12] tracking-[-0.045em]">{practice.title}</h3><p className="mt-6 max-w-[330px] text-[14px] leading-7 text-[#211c17]/68">{practice.description}</p></div><div className="mt-8 border-t border-[#211c17]/14 pt-5">{practice.points.map((point) => <div key={point} className="flex items-center justify-between border-b border-[#211c17]/10 py-2.5 text-[13px] text-[#211c17]/68 last:border-b-0"><span>{point}</span><span className="text-[#211c17]/35">↗</span></div>)}</div></div></motion.div>
