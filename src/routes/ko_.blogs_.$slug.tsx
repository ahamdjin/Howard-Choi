...
export const Route = createFileRoute("/ko/blogs/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    const seo = buildSeo({ title: `${loaderData.title} | Buena Park Injury Lawyer`, description: loaderData.excerpt, path, alternatePath: `/blogs/${loaderData.slug}`, locale: "ko-US", type: "article", image: loaderData.image });
    return {
      ...seo,
      ...
      scripts: [
        { type: "application/ld+json", children: JSON.stringify(articleJsonLd({ title: loaderData.title, description: loaderData.excerpt, path, image: loaderData.image, publishedAt: loaderData.publishedAt, locale: "ko-US" })) },
        { type: "application/ld+json", children: JSON.stringify(breadcrumbJsonLd([{ name: "홈", path: "/ko" }, { name: "법률 블로그", path: "/ko/blogs" }, { name: loaderData.title, path }])) },
      ],
    };
  },
